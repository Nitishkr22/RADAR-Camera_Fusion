...
def image_callback(msg):
    global cv_image, current_bounding_boxes
    bridge = CvBridge()
    ...
    if cv_image is not None:
        ...
        for bbox in current_bounding_boxes:
            ...
            for (x, y), radar_range in zip(predictions, rangep):
                p = int(x)
                q = int(y)
                if x_min <= p <= x_max and y_min <= q <= y_max:
                    if 0 <= p < cv_image.shape[0] and 0 <= q < cv_image.shape[1]:
                        cv2.circle(cv_image, (p, q), circle_radius, colors, -1)
                        text = f"{radar_range:.2f} m"
                        cv2.putText(cv_image, text, (p, q - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors, 2)
